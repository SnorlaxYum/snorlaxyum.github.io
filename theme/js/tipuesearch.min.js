(function($){$.fn.tipuesearch=function(options){var set=$.extend({'show':7,'newWindow':false,'showURL':true,'showTitleCount':true,'minimumLength':1,'descriptiveWords':25,'highlightTerms':true,'highlightEveryTerm':false,'mode':'static','liveDescription':'*','liveContent':'*','contentLocation':'tipuesearch/tipuesearch_content.json','debug':false},options);return this.each(function(){var tipuesearch_in={pages:[]};$.ajaxSetup({async:false});var tipuesearch_t_c=0;if(set.mode=='live')
...
{t_d+=' ...';}
out+='<div class="tipue_search_content_text">'+t_d+'</div><hr>';}}
l_o++;}
if(c>set.show)
{var pages=Math.ceil(c/set.show);var page=(start/set.show);out+='<div id="tipue"><table><tr>';if(start>0)
{out+='<td style="text-align:left;width:5ch"><a class="tipue_search_foot_box" id="'+(start-set.show)+'_'+replace+'">'+tipuesearch_string_6+'</a></td>';}
if(page<=2)
{var p_b=pages;if(pages>3)
{p_b=3;}
for(var f=0;f<p_b;f++)
{if(f==page)
{out+='<td style="text-align:center"><b>Page '+(f+1)+'</b></td>';}
else
{out+='';}}}
else
...
{out+='';}}}
if(page+1!=pages)
{out+='<td style="text-align:right;width:5ch"><a class="tipue_search_foot_box" id="'+(start+set.show)+'_'+replace+'">'+tipuesearch_string_7+'</a></td>';}
out+='</tr></table></div>';}}
else
